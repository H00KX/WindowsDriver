...
#define LinkName	L"\\DosDevices\\COM7"
...
NTSTATUS HelloWDMAddDevice(PDRIVER_OBJECT pDriverObject, PDEVICE_OBJECT PhysicalDeviceObject)
{
...
NTSTATUS HelloWDMCreate(PDEVICE_OBJECT fdo, PIRP pIrp)
{
...
NTSTATUS HelloWDMPnp(PDEVICE_OBJECT fdo, PIRP pIrp)
{
...
void HelloWDMUnload(PDRIVER_OBJECT pDriverObject)
{
...
VOID DriverCancelWaitIrp(PDEVICE_OBJECT pDeviceObject, PIRP pIrp)
{
...
VOID DriverCheckEvent(PDEVICE_EXTENSION pExtension, ULONG events)
{
	...
	if ((pExtension->pWaitIrp != NULL) && (events != 0))
	{
		...
		else  //如果irp已经被cancen掉，就不用IoCompleteRequest了
		{
			pOldWaitIrp = NULL;
		}
	}
...
NTSTATUS HelloWDMDispatchControlp(PDEVICE_OBJECT fdo, PIRP pIrp)
{
	...
	case IOCTL_SERIAL_SET_BAUD_RATE:
	{
		pdx->BaudRate = ((PSERIAL_BAUD_RATE)(pIrp->AssociatedIrp.SystemBuffer))->BaudRate;
		pIrp->IoStatus.Information = 0;
		break;
	}
	...
	case IOCTL_SERIAL_GET_MODEMSTATUS:
	{
		...
		*(PULONG)pIrp->AssociatedIrp.SystemBuffer = (Cts ? SERIAL_CTS_STATE : 0) | (Dsr ? SERIAL_DSR_STATE : 0) | (Dcd ? SERIAL_DCD_STATE : 0);
		pIrp->IoStatus.Information = sizeof(ULONG);
	...
	case IOCTL_SERIAL_SET_TIMEOUTS:
	{
		pdx->Timeouts = *((PSERIAL_TIMEOUTS)(pIrp->AssociatedIrp.SystemBuffer));
		break;
	}

	case IOCTL_SERIAL_GET_TIMEOUTS:
	{
		*((PSERIAL_TIMEOUTS)pIrp->AssociatedIrp.SystemBuffer) = pdx->Timeouts;
	...
	case IOCTL_SERIAL_SET_LINE_CONTROL:
	{
		pdx->Lc = *((PSERIAL_LINE_CONTROL)(pIrp->AssociatedIrp.SystemBuffer));
		break;
	}

	case IOCTL_SERIAL_GET_LINE_CONTROL:
	{
		*((PSERIAL_LINE_CONTROL)(pIrp->AssociatedIrp.SystemBuffer)) = pdx->Lc;
	...
	case IOCTL_SERIAL_SET_WAIT_MASK:
	{
		...
		pdx->EventMask = *(PULONG)pIrp->AssociatedIrp.SystemBuffer;
	...
	case IOCTL_SERIAL_GET_WAIT_MASK:
	{
		*(PULONG)pIrp->AssociatedIrp.SystemBuffer = pdx->EventMask;
	...
	case IOCTL_SERIAL_WAIT_ON_MASK:
	{
		...
		{
			pIrp->IoStatus.Information = sizeof(ULONG);
			*(PULONG)pIrp->AssociatedIrp.SystemBuffer = pdx->EventMask & pdx->HistoryEvents;
			pdx->HistoryEvents = 0;
			ntStatus = STATUS_SUCCESS;
		}
		else
...
VOID DriverCancelCurrentReadIrp(PDEVICE_OBJECT DeviceObject, PIRP pIrp)
{
...
NTSTATUS HelloWDMRead(PDEVICE_OBJECT fdo, PIRP pIrp)
{
	...
	PCHAR pBuf = (PCHAR)pIrp->AssociatedIrp.SystemBuffer;
...
NTSTATUS HelloWDMWrite(PDEVICE_OBJECT fdo, PIRP pIrp)
{
	...
	PUCHAR pData = (PUCHAR)pIrp->AssociatedIrp.SystemBuffer;
